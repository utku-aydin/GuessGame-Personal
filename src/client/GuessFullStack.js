...
$('form#form2').submit(function( event ) {
  ...
  `<div class="spinner-border" role="status">
	...
	fetch("http://localhost:8080/api/rounds/" + id)
	.then(response => {
    if (response.ok) {
      tableDisplay.innerHTML =
        `<table>
...
function beginGame() {
  ...
  fetch('http://localhost:8080/api/begin', {
   method: 'POST', // or 'PUT'
   headers: {
     'Content-Type': 'application/json',
   },
...
function validateIdInput(id, tableDisplay, idInput) {
  ...
  if (id < minMax.min || id > minMax.max) {
    tableDisplay.innerHTML =
      `Input a game ID between ${minMax.min} and ${minMax.max}.`;
    idInput.style.background = "#DE4118";
    return false;
  }
